...
from build_clang.helpers import BUILD_CLANG_SCRIPTS_ROOT_PATH
...
def rel_to_repo_root(file_path: str) -> str:
    return os.path.relpath(
        ...
        os.path.realpath(BUILD_CLANG_SCRIPTS_ROOT_PATH))
...
def check_file(file_path: str, check_type: str) -> CheckResult:
    ...
    if check_type == 'mypy':
        ...
        what_to_import = None
        if rel_path.startswith('src/build_clang/'):
            where_to_import_from = 'build_clang'
            # Assuming a one-level hierarchy, i.e. that the build_clang directory does not contain
            # any module subdirectories with Python files.
        else:
        ...
        args = ['pycodestyle',
                '--config=%s' % os.path.join(BUILD_CLANG_SCRIPTS_ROOT_PATH, 'pycodestyle.cfg')]
    else:
...
def check_python_code() -> bool:
    ...
    input_file_paths = (
        glob.glob(os.path.join(BUILD_CLANG_SCRIPTS_ROOT_PATH, 'src', 'build_clang', '*.py')) +
        glob.glob(os.path.join(BUILD_CLANG_SCRIPTS_ROOT_PATH, '*.py'))
    )

    for dirpath, dirnames, filenames in os.walk(
            os.path.join(BUILD_CLANG_SCRIPTS_ROOT_PATH, 'python')):
